    ...
    for (const subject of subjects) {
        ...
        for (const st of subjectTeachers) {
            ...
            const averageMarks = marks.length > 0
                ? marks.reduce((sum, mark) => sum + (mark.score || 0), 0) / marks.length
                : 0;
    ...
    for (const teacher of teachers) {
        ...
        for (const subClass of subClassMap.values()) {
            ...
            const averageMarks = classMarks.length > 0
                ? classMarks.reduce((sum, mark) => sum + (mark.score || 0), 0) / classMarks.length
                : 0;
        ...
        const passRate = allMarks.length > 0
            ? (allMarks.filter(mark => (mark.score || 0) >= 10).length / allMarks.length) * 100
            : 0;
        const excellentRate = allMarks.length > 0
            ? (allMarks.filter(mark => (mark.score || 0) >= 16).length / allMarks.length) * 100
            : 0;
...
export async function getSubjectPerformance(
    ...
    for (const subject of subjects) {
        ...
        const passRate = subjectMarks.length > 0
            ? (subjectMarks.filter(mark => (mark.score || 0) >= 10).length / subjectMarks.length) * 100
            : 0;

        const excellentRate = subjectMarks.length > 0
            ? (subjectMarks.filter(mark => (mark.score || 0) >= 16).length / subjectMarks.length) * 100
            : 0;
        ...
        subjectMarks.forEach(mark => {
            ...
            sequenceMap.get(key).marks.push(mark.score || 0);
        });
...
export async function getDepartmentAnalytics(
    ...
    for (const subject of subjects) {
        ...
        allMarks.forEach(mark => {
            ...
            classPerformanceMap.get(classId).marks.push(mark.score || 0);
        });
        ...
        allMarks.forEach(mark => {
            ...
            teacherData.marks.push(mark.score || 0);
        ...
        const monthlyTrends = Array.from(
            allMarks.reduce((map, mark) => {
                ...
                map.get(month).marks.push(mark.score || 0);
...
export async function getTeacherPerformance(
    ...
    const averageScore = totalMarks > 0
        ? marks.reduce((sum, mark) => sum + (mark.score || 0), 0) / totalMarks
        : 0;

    const passRate = totalMarks > 0
        ? (marks.filter(mark => (mark.score || 0) >= 10).length / totalMarks) * 100
        : 0;

    const excellentRate = totalMarks > 0
        ? (marks.filter(mark => (mark.score || 0) >= 16).length / totalMarks) * 100
        : 0;
    ...
    const subjectPerformance = marks.reduce((acc, mark) => {
        ...
        acc[subjectId].marks.push(mark.score || 0);
        return acc;
    ...
    const classPerformance = marks.reduce((acc, mark) => {
        ...
        acc[subClassId].marks.push(mark.score || 0);
        return acc;
