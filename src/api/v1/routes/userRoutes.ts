...
router.post('/create-with-role', authenticate, authorize(['SUPER_MANAGER']), createUserWithRole);

// User CRUD operations (requires authentication, some require specific roles)
router.get('/', authenticate, authorize(['SUPER_MANAGER']), getAllUsers);
router.post('/', authenticate, authorize(['SUPER_MANAGER']), createUser);
...
router.get('/:parentId/students', authenticate, authorize(['SUPER_MANAGER']), getStudentsForParent);
// Note: Add logic in controller/service to ensure PARENT can only access their own students if parentId matches req.user.id

router.get('/:id', authenticate, authorize(['SUPER_MANAGER']), getUserById);
router.put('/:id', authenticate, authorize(['SUPER_MANAGER']), updateUser);
router.delete('/:id', authenticate, authorize(['SUPER_MANAGER']), deleteUser); // Only SUPER_MANAGER can delete

// Role management
router.post('/:id/roles', authenticate, authorize(['SUPER_MANAGER']), assignRole); // Single role assignment
router.delete('/:id/roles', authenticate, authorize(['SUPER_MANAGER']), removeRole); // Remove role (specify role in body)
router.put('/:id/roles/academic-year', authenticate, authorize(['SUPER_MANAGER']), setUserRolesForCurrentAcademicYear); // New route for setting roles
router.delete('/:id/roles/:roleId', authenticate, authorize(['SUPER_MANAGER']), removeRole); // RoleId here is the UserRole record ID
...
router.post('/:userId/assignments/vice-principal', authenticate, authorize(['SUPER_MANAGER']), assignVicePrincipal);
// Remove VP from Subclass (Requires subClassId in path. Defaults to current year if academicYearId query param omitted)
router.delete('/:userId/assignments/vice-principal/:subClassId', authenticate, authorize(['SUPER_MANAGER']), removeVicePrincipal);

// Assign DM to Subclass (Defaults to current year if academicYearId is omitted in body)
router.post('/:userId/assignments/discipline-master', authenticate, authorize(['SUPER_MANAGER']), assignDisciplineMaster);
// Remove DM from Subclass (Requires subClassId in path. Defaults to current year if academicYearId query param omitted)
router.delete('/:userId/assignments/discipline-master/:subClassId', authenticate, authorize(['SUPER_MANAGER']), removeDisciplineMaster);
